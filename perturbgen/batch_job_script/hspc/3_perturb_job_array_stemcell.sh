...
#BSUB -J hspc_perturb_cluster_[1-21]%4 # job array with 1000 jobs, max 16 running at the same time
#BSUB -n 4 # number of cores
#BSUB -G team361 # groupname for billing team361
#BSUB -cwd /lustre/scratch126/cellgen/lotfollahi/kl11/ # working directory
#BSUB -o T_perturb/perturbgen/logs/perturb_tf_stem_%J_%I.out # output file
#BSUB -e T_perturb/perturbgen/logs/perturb_tf_stem_%J_%I.err # error file
...
PERTURBED_GENE=$(sed -n "${LSB_JOBINDEX}p" $cwd/T_perturb/perturbgen/configs/eval/HSPC/perturb_TF.txt)

echo "[$(date)] Starting job index $LSB_JOBINDEX"
echo "Current gene to be perturbed: $PERTURBED_GENE"

CONFIG_PATH="T_perturb/perturbgen/configs/eval/HSPC/mask_stem_inference_perturbation_jobarray.yaml"
TMP_CONFIG_PATH="T_perturb/perturbgen/configs/eval/HSPC/mask_stem_generate_perturbation_${PERTURBED_GENE}_${LSB_JOBID}.yaml"
