...
def pairing_src_to_tgt_cells(
    ...
    if pairing_mode == 'stratified':
        ...
        adata_grouped = adata_subset_.obs[
            adata_subset_.obs.groupby(['donor_cellgen_harm', 'cell_type_cellgen_harm'])[pairing_obs].transform(
                'nunique'
            )
            == 4
        ]
        dropped_donors = (
            adata_subset.obs['donor_cellgen_harm'].nunique() - adata_grouped['donor_cellgen_harm'].nunique()
        )
        print(f'dropped {dropped_donors} donors')
        resting_cells = adata_grouped.loc[adata_grouped[pairing_obs] == 'normal', :]
        grouped = adata_grouped.groupby(['donor_cellgen_harm', 'cell_type_cellgen_harm'])
