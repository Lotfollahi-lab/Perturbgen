...
class CytoMeisterTrainer(LightningModule):
    ...
    def forward(self, batch, generate: bool = False):
        """
        batch must contain:
        - 'src_input_ids': Tensor[B, L_src]
        - 'tgt_input_ids_t{t}': Tensor[B, L_tgt]  (for all t in 1..n)
        - optionally 'tgt_time_step', 'generate_id_dict', 'generate_pad_dict'
        generate: 
        - False = training (single-pass with dropout)
        - True  = inference (two-pass CFG)
        Returns (outputs, tgt_input_id_dict)
        """
        if self.classifier_free_guidance:
            # 1) pick the one time-step
            tgt_time_step = batch.get('tgt_time_step', self.pred_tps[0])

            # 2) raw targets for that step
            orig_tgt = batch[f'tgt_input_ids_t{tgt_time_step}']  # [B, L]

            # 3) build condition‐ids [B, 1]
            ...
            # 4) assemble dict (no manual concatenation!)
            tgt_input_id_dict = {
                f'tgt_input_ids_t{tgt_time_step}': orig_tgt
            }

            if generate:
                # --- inference: true 2-pass CFG blend ---
                outputs = self.transformer.forward_with_cond_scale(
                    cond_scale        = self.transformer.cfg_scale,
                    src_input_id      = batch['src_input_ids'],
                    tgt_input_id_dict = tgt_input_id_dict,
                    not_masked        = self.return_embeddings,
                    cond_dict         = cond_ids.squeeze(1),          # [B]
                    generate_id_dict  = batch.get('generate_id_dict'),
                    generate_pad_dict = batch.get('generate_pad_dict'),
                    tgt_time_step     = tgt_time_step,
                )
            else:
                # --- training: single-pass with dropout of cond token ---
                orig_ctx = self.transformer.context_mode
                self.transformer.context_mode = False

                outputs = self.transformer.forward(
                    src_input_id      = batch['src_input_ids'],
                    tgt_input_id_dict = tgt_input_id_dict,
                    cond_dict         = cond_ids.squeeze(1),
                ...
                self.transformer.context_mode = orig_ctx
