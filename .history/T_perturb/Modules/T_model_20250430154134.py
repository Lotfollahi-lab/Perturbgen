        ...
        if self.classifier_free_guidance:
            # build one flat vocab of all condition tokens + a special "<null>"
                ...
                for name, vocab in condition_dict.items():
                    # vocab: token → orig_id (int)
                    # first invert to map orig_id → local index [0..N-1]
                    inv = {orig_id: i for i, (_tok, orig_id) in enumerate(vocab.items())}
                    # now build a mapping orig_id → global_index = offset + local_index
                    condition_dict_[name] = {
                        orig_id: offset + inv[orig_id] for orig_id in vocab.values()
                    }
                    offset += len(vocab)

                # reserve one extra slot at the end for our "<null>" id
                self.null_index = offset
                condition_dict_['uncondition'] = { self.null_index: self.null_index }
    ...
    def forward(
        ...
        **kwargs,
        ...
        if tgt_input_id_dict:
            # only pad for the time‐steps we actually provided
            provided_tps = [
                int(key.split('_t')[-1])
                for key in tgt_input_id_dict.keys()
            ]
    ...
    def generate_sequence(
        ...
        **kwargs,
        ...
        for iteration, steps_until_x0 in zip(
            ...
            outputs = demask_fn.forward_with_cond_scale( #forward_with_cond_scale
