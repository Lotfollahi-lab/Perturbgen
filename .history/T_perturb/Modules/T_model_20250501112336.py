    ...
    def forward(
        ...
        **kwargs,
        ...
        outputs: `dict`
            Output dictionary
        '''
        print("***cond_drop_prob***",cond_drop_prob)
        ...
        for tgt_time_step in sorted_time_steps:
            ...
            if self.classifier_free_guidance:
                # Optional dropout: just mask condition tokens in-place in tgt_pad if needed
                if cond_drop_prob > 0:
                    cond_len = cond_dict.shape[1]
                    dropout_mask = prob_mask_like((tgt_pad.shape[0], cond_len), cond_drop_prob, device=tgt_pad.device)
                    tgt_pad = torch.cat([dropout_mask, tgt_pad[:, cond_len:]], dim=1)
                assert tgt_input_id.shape[1] == tgt_pad.shape[1], \
                    f"Shape mismatch: tgt_input_id {tgt_input_id.shape}, tgt_pad {tgt_pad.shape}"
                # if labels is not None:
                #     labels = F.pad(labels, (cond_dict.shape[1], 0), value=-100)
