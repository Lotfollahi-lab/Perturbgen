    ...
    def __init__(
        ...
        if self.classifier_free_guidance:
            ...
            for name, vocab in condition_dict.items():
                # vocab: Dict[token_str, orig_id]. We ignore orig_id now.
                # Build a flat mapping from each token_str → (offset + local_index)
                flat_map = {}
                for local_idx, token_str in enumerate(vocab.keys()):
                    flat_map[token_str] = offset + local_idx
                condition_dict_[name] = flat_map
                offset += len(vocab)

            # reserve one extra slot for "<null>"
