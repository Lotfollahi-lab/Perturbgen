        ...
        **kwargs,
        ...
        for tgt_time_step in sorted_time_steps:
            ...
            if self.classifier_free_guidance:
                ...
                tgt_pad = torch.cat([dropout_mask, tgt_pad], dim=1)

                if labels is not None and cond_drop_prob < 1.0:
                    labels = F.pad(labels, (cond_len, 0), value=-100)
                # Final consistency checks
                assert tgt_input_id.shape[1] == tgt_pad.shape[1], \
                    f"Shape mismatch: tgt_input_id {tgt_input_id.shape}, tgt_pad {tgt_pad.shape}"
                assert labels is None or labels.shape == tgt_input_id.shape, \
                    f"Shape mismatch: labels {labels.shape}, input {tgt_input_id.shape}"
