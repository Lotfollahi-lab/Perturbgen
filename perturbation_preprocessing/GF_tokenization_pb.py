...
from T_perturb.src.utils import (
    ...
    pairing_src_to_tgt_cells_perturbation,
    str2bool,
    subset_adata,
    tokenid_mapping,
)
...
adata = sc.read_h5ad('/lustre/scratch126/cellgen/team205/bair/perturbench/perturbench_data/norman19_processed.h5ad')
...
(adata_subset, token_id_to_row_id_dict, row_id_to_gene_name) = tokenid_mapping(
    adata,
    '/lustre/scratch126/cellgen/team361/hk11/cytoimmgen/.venv/src/geneformer/geneformer/token_dictionary_gc95M.pkl',
    exclude_non_GF_genes=args.exclude_non_GF_genes,
)
# save mapping dictionnary
with open(
    f'/lustre/scratch126/cellgen/team205/bair/perturbench/perturbench_data/norman/'
...
adata_subset.write_h5ad(
    f'{paired_h5ad_dir}/norman_{args.gene_filtering_mode}.h5ad'
)
...
tk.tokenize_data(
    ...
    f'norman_{args.gene_filtering_mode}',  # name of output file
...
dataset = load_from_disk(
    f'{output_dir}/norman_{args.gene_filtering_mode}.dataset'
)
...
# Pairing control to perturbed cells and tokenize  datasets
cell_pairings = pairing_src_to_tgt_cells_perturbation(
    adata_subset=adata_subset,
    pairing_mode=args.pairing_mode,
    perturbation_obs='perturbation',  # Updated to use perturbation column
    seed_no=seed_no,
)
...
# Directory setup
paired_dataset_dir = (
    f'/lustre/scratch126/cellgen/team205/bair/perturbench/perturbench_data/norman/dataset_{args.gene_filtering_mode}'
)
...
token_id_to_row_id_dict = pickle.load(
    open(
        f'/lustre/scratch126/cellgen/team205/bair/perturbench/perturbench_data/norman/tokenid_to_rowid_{args.gene_filtering_mode}.pkl',
        'rb',
    )
)
...
# Mapping dataset for tokenization
...
# Prepare directories for control and combined perturbed datasets
...
# Save control data (source)
control_pairings = cell_pairings['control']
adata_control = subset_adata(adata_subset, control_pairings)
adata_control.write_h5ad(f'{src_adata_dir}/control.h5ad')

if args.src_mode == 'Geneformer':
    dataset_control = dataset.select(control_pairings)
    dataset_control.save_to_disk(f'{src_dataset_dir}/control.dataset')
else:
    ...
    dataset_control = dataset_mapped.select(control_pairings)
    dataset_control.save_to_disk(f'{src_adata_transf_dir}/control.dataset')

# Save all perturbed data together (target)
perturbed_pairings = [idx for pert in cell_pairings.keys() if pert != 'control' for idx in cell_pairings[pert]]
adata_perturbed = subset_adata(adata_subset, perturbed_pairings)
adata_perturbed.write_h5ad(f'{tgt_adata_dir}/perturbation.h5ad')

dataset_perturbed = dataset_mapped.select(perturbed_pairings)
dataset_perturbed.save_to_disk(f'{tgt_dataset_dir}/perturbation.dataset')