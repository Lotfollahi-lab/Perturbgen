...
from T_perturb.Modules.T_model_encoding import CellGen, CountDecoder
...
class CellGenTrainer(LightningModule):
    ...
    def test_step(self, batch, *args, **kwargs):
        if self.return_embeddings:
            if self.perturbation_mode == 'KO':
                ...
                outputs_nomask = self.forward(batch)
                
                perturbation_genes_ids = {v: k for k, v in self.subset_tokenid_to_genename.items() if v in self.perturbation_genes}
                for gene in self.perturbation_genes:
                    for time_step in self.time_steps:
                        ...
                        # masking perturbation gene
                        masked_batch = batch.copy()
                        masked_batch[f'tgt_input_ids_t{time_step}'] = batch[f'tgt_input_ids_t{time_step}'].clone()
                        if gene == 'first_gene':
                            first_token_ids = batch[f'tgt_input_ids_t{time_step}'][:, 0]
                            masked_batch[f'tgt_input_ids_t{time_step}'][:, 0] = self.transformer.mask_token
                            # masked_batch['src_input_ids'] = masked_batch['src_input_ids'].clone()
                            # masked_batch['src_input_ids'][:, 0] = 1
                        else:
                            masked_batch[f'tgt_input_ids_t{time_step}'][token_ids == perturbation_genes_ids[gene]] = self.transformer.mask_token
                        outputs_mask = self.forward(masked_batch)
                        
                        # get embeddings
                        dec_embedding = outputs_nomask['dec_embedding'][time_step]
                        cls_embeddings = dec_embedding[:, 0, :]
                        gene_embeddings = dec_embedding[:, 1:, :]
                        # compute mean gene embeddings
                        mean_gene_embeddings = torch.mean(gene_embeddings, dim=1)
                
                        # get perturbation embeddings
                        pert_dec_embedding = outputs_mask['dec_embedding'][time_step]
                        pert_cls_embeddings = pert_dec_embedding[:, 0, :]
                        pert_gene_embeddings = pert_dec_embedding[:, 1:, :]
                        pert_mean_gene_embeddings = torch.mean(pert_gene_embeddings, dim=1)
                        
                        if gene == 'first_gene':
                            # Calculate cosine similarity for the filtered embeddings
                            cls_cosine_similarities = F.cosine_similarity(cls_embeddings, pert_cls_embeddings, dim=1)
                            mean_cosine_similarities = F.cosine_similarity(mean_gene_embeddings, pert_mean_gene_embeddings, dim=1)
        
                            # Store the results
                            self.test_dict['batch'].append(batch['combined_batch'].detach().cpu())
                            self.test_dict['cell_idx'].append(cell_ids)
                            self.test_dict['true_counts'].append(batch[f'tgt_counts_t{time_step}'].detach().cpu())
                            self.test_dict['cls_cosine_similarities'].append(cls_cosine_similarities.detach().cpu())
                            self.test_dict['mean_cosine_similarities'].append(mean_cosine_similarities.detach().cpu())
                            self.test_dict['cls_embeddings'].append(cls_embeddings.detach().cpu())
                            # self.test_dict['gene_embeddings'].append(gene_embeddings.detach().cpu())
                            self.test_dict['mean_gene_embeddings'].append(mean_gene_embeddings.detach().cpu())
                            self.test_dict['perturbed_cls_embeddings'].append(pert_cls_embeddings.detach().cpu())
                            # self.test_dict['perturbed_gene_embeddings'].append(pert_gene_embeddings.detach().cpu())
                            self.test_dict['perturbed_mean_gene_embeddings'].append(pert_mean_gene_embeddings.detach().cpu())
                            self.test_dict['perturbed_gene'].append([self.subset_tokenid_to_genename[token_id.item()] for token_id in first_token_ids])
                            self.test_dict['time_step'].append([time_step for i in range(cls_embeddings.size(0))])
                            
                            if len(self.var_list) > 0:
                                for var in self.var_list:
                                    self.test_dict[var].append(batch[f'{var}_t{time_step}'])
                        else:
                            # Create a boolean mask for cells containing the specific perturbed gene
                            cond_perturb = (token_ids == perturbation_genes_ids[gene]).sum(1) > 0
                            cond_perturb_indx = torch.where(cond_perturb)[0]

                            # Calculate cosine similarity for the filtered embeddings
                            cls_cosine_similarities = F.cosine_similarity(cls_embeddings[cond_perturb], pert_cls_embeddings[cond_perturb], dim=1)
                            mean_cosine_similarities = F.cosine_similarity(mean_gene_embeddings[cond_perturb], pert_mean_gene_embeddings[cond_perturb], dim=1)
        
                            # Store the results
                            self.test_dict['batch'].append(batch['combined_batch'][cond_perturb].detach().cpu())
                            self.test_dict['cell_idx'].append([cell_ids[i.item()] for i in cond_perturb_indx])
                            self.test_dict['true_counts'].append(batch[f'tgt_counts_t{time_step}'][cond_perturb].detach().cpu())
                            self.test_dict['cls_cosine_similarities'].append(cls_cosine_similarities.detach().cpu())
                            self.test_dict['mean_cosine_similarities'].append(mean_cosine_similarities.detach().cpu())
                            self.test_dict['cls_embeddings'].append(cls_embeddings[cond_perturb].detach().cpu())
                            # self.test_dict['gene_embeddings'].append(gene_embeddings[cond_perturb].detach().cpu())
                            self.test_dict['mean_gene_embeddings'].append(mean_gene_embeddings[cond_perturb].detach().cpu())
                            self.test_dict['perturbed_cls_embeddings'].append(pert_cls_embeddings[cond_perturb].detach().cpu())
                            # self.test_dict['perturbed_gene_embeddings'].append(pert_gene_embeddings[cond_perturb].detach().cpu())
                            self.test_dict['perturbed_mean_gene_embeddings'].append(pert_mean_gene_embeddings[cond_perturb].detach().cpu())
                            self.test_dict['perturbed_gene'].append([gene for i in cond_perturb_indx])
                            self.test_dict['time_step'].append([time_step for i in cond_perturb_indx])
                            
                            if len(self.var_list) > 0:
                                for var in self.var_list:
                                    self.test_dict[var].append([batch[f'{var}_t{time_step}'][i.item()] for i in cond_perturb_indx])
            else:
                outputs = self.forward(batch)
                for time_step in self.time_steps:
                    token_ids = batch[f'tgt_input_ids_t{time_step}']
    ...
    def on_test_epoch_end(self):
        if self.return_embeddings:
            ...
            if self.perturbation_mode == 'KO':
                cls_cosine_similarities = torch.cat(self.test_dict['cls_cosine_similarities'])
                mean_cosine_similarities = torch.cat(self.test_dict['mean_cosine_similarities'])
                mean_gene_embeddings = torch.cat(self.test_dict['mean_gene_embeddings'])
                perturbed_cls_embeddings = torch.cat(self.test_dict['perturbed_cls_embeddings'])
                # perturbed_gene_embeddings = torch.cat(self.test_dict['perturbed_gene_embeddings'])
                perturbed_mean_gene_embeddings = torch.cat(self.test_dict['perturbed_mean_gene_embeddings'])
                ...
                adata = ad.AnnData(
                    ...
                    obsm={
                        ...
                        'perturbed_mean_gene_embeddings': perturbed_mean_gene_embeddings.numpy(),
                    },
                )
            else:
