...
def tokenid_mapping(
    ...
    print(f'Number of genes dropped: {adata.n_vars - adata_subset.n_vars}')
    print(f'Number of genes now: {adata_subset.n_vars}')
...
def subset_dataset(
    dataset: DatasetDict,
    cell_pairings: List[int],
) -> DatasetDict:
    """
    Description:
    ------------
    Subset the Huggingface DatasetDict based on a list of cell indices.
    ...
    cell_pairings: `List[int]`
        List of cell indices to include in the subset.

    Returns:
    --------
    subsetted_dataset: `DatasetDict`
        The subsetted dataset containing only the specified cells.
    """
    # Ensure cell_pairings are unique
    cell_pairings = list(set(cell_pairings))
    subsetted_dataset = dataset.select(cell_pairings)
...
def pairing_src_to_tgt_cells_perturbation(
    adata_subset: sc.AnnData,
    pairing_mode: str,
    perturbation_obs: str = 'perturbation',
    seed_no: int = 42,
):
    """
    Description:
    ------------
    This function pairs control cells to perturbed cells based on the 'perturbation' column.
    ...
    perturbation_obs: `str`
        Column name in `adata_subset.obs` that contains perturbation labels.
    seed: `int`
        Seed for random number generator.
    ...
    # Separate control cells from perturbed cells
    control_cells = adata_subset_.obs[adata_subset_.obs[perturbation_obs] == 'control']
    perturbed_cells = adata_subset_.obs[adata_subset_.obs[perturbation_obs] != 'control']
    
    # Initialize dictionary to store cell pairing
    cell_pairings: Dict[str, List[int]] = {'control': [], 'perturbed': []}
    
    # Stratified pairing
    if pairing_mode == 'stratified':
        # Pair cells within the same Donor and Cell_type
        grouped_perturbed = perturbed_cells.groupby(['Donor', 'Cell_type'])
        grouped_control = control_cells.groupby(['Donor', 'Cell_type'])

        for (donor, cell_type), group_perturbed in grouped_perturbed:
            # Check if the same donor and cell type exist in control cells
            if (donor, cell_type) in grouped_control.groups:
                group_control = grouped_control.get_group((donor, cell_type))
                
                for idx_perturbed in group_perturbed.index:
                    # Randomly sample from control cells with replacement
                    idx_control = np.random.choice(group_control.index)
                    cell_pairings['control'].append(idx_control)
                    cell_pairings['perturbed'].append(idx_perturbed)
                    
    # Random pairing
    elif pairing_mode == 'random':
        # Randomly sample control cells to match the number of perturbed cells
        cell_pairings['perturbed'] = perturbed_cells.index.tolist()
        cell_pairings['control'] = np.random.choice(
            control_cells.index, len(perturbed_cells), replace=True
        ).tolist()
        
        # Print the number of paired perturbed and control cells
        print(f"Number of paired perturbed cells: {len(cell_pairings['perturbed'])}")
        print(f"Number of paired control cells: {len(cell_pairings['control'])}")
