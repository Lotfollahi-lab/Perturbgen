...
class DualPositionalEncoding(nn.Module):
    def __init__(self, d_model, max_seq_length, n_time_steps, mode='GF_fine_tuned'):
        ...
        Description:
        ------------
        Positional encoding for the transformer model with learnable gene positional encoding
        and fixed time positional encoding.
        Parameters:
        -----------
        d_model: `int`
            Token embedding dimension.
        ...
        '''
        super(DualPositionalEncoding, self).__init__()
        
        # Gene positional encoding (learnable)
        self.gene_position_embeddings = nn.Embedding(max_seq_length, d_model)
        
        # Time positional encoding (fixed)
        self.max_seq_length = max_seq_length
        if mode in ['GF_frozen', 'GF_fine_tuned']:
            total_seq_length = n_time_steps * max_seq_length
        elif mode == 'Transformer_encoder':
    ...
    def forward(self, x, tgt_time_step=None):
        # Fixed time positional encoding
        if self.mode in ['GF_frozen', 'GF_fine_tuned']:
            tgt_time_step_ = tgt_time_step - 1
        elif self.mode == 'Transformer_encoder':
            tgt_time_step_ = tgt_time_step
        ...
        if tgt_time_step is not None:
            start_pos = (tgt_time_step_) * self.max_seq_length
            end_pos = start_pos + x.size(1)
            time_pe = self.pe[:, start_pos:end_pos]
        else:
            time_pe = self.pe[:, : x.size(1)]

        # Learnable gene positional encoding
        batch_size, seq_len, _ = x.size()
        position_ids = torch.arange(seq_len, device=x.device).expand(batch_size, seq_len)
        gene_pe = self.gene_position_embeddings(position_ids)
...
class Geneformerwrapper(nn.Module):
    def __init__(
        self,
        model_path='/lustre/scratch126/cellgen/team361/hk11/cytoimmgen/.venv/src/geneformer/',
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.02,
        d_ff: int = 512,
        position_embedding: Literal['sinusoidal', 'learnt', 'dual'] = 'dual',
    ):
        super().__init__()
        ...
        elif self.position_embedding == 'dual':
            self.positional_encoding = DualPositionalEncoding(
                d_model=d_model,
                max_seq_length=max_seq_length,
                n_time_steps=n_time_steps,
                mode='Transformer_encoder',
...
class CellGen(nn.Module):
    def __init__(
        ...
        mode: `str`
            Mode of the encoder.
            Options: ['GF_frozen', 'GF_fine_tuned', 'Transformer_encoder']
        position_embedding: `str` (default: 'learnt', 'dual')
            Positional encoding type: ['sinusoidal', 'learnt', 'dual'].
        Returns:
        --------
        outputs: `dict`
